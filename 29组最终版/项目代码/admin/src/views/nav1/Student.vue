...
<script>
	...
	export default {
		...
		methods: {
			...
			getSex () {
				this.$axios.get('http://www.xhw8888.com/index/app/getSexlist', {
                    emulateJSON: true
                }, {
                    headers: {
			...
			getXueli () {
				this.$axios.get('http://www.xhw8888.com/index/app/getXuelilist', {
                    emulateJSON: true
                }, {
                    headers: {
                    'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8'
			...
			getStuClass: function (a) {
				this.$axios.get('http://47.113.124.225:8083/c/getAll',)
			...
			getUsers() {
				...
				this.$axios.get('http://47.113.124.225:8083/Student/Page',{params:{indexpage:para.indexpage,everypage:para.everypage}})
				.then(function (response) {
					//console.log(response.data);

					// second_this.users = response.data["data"];   //交给前端显示

					second_this.getHAHA(response.data["data"])
			...
			getHAHA(data) {
				var second_this = this;
				this.$axios.post('http://www.xhw8888.com/index/app/tehPage',qs.stringify(
				{
					data:data,
				}))
				.then(function (response) {
					second_this.users = response.data;
				})
				.catch(function (error) {
					console.log(error);
				});
			...
			findUser() {
				...
				this.listLoading = true;  //获取数据成功
				var second_this = this;
				this.$axios.get('http://47.113.124.225:8083/User?',{params:{stno:para.name}})
				.then(function (response) {
					//console.log(response.data);
			...
			handleDel: function (index, row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					this.$axios.delete('http://47.113.124.225:8083/User', {params:{stno:para.stno}})
					    .then(function (response) {
							console.log(response.data);
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							formData.append('classId',editParams.classId);
							...
							sec_this.$axios.delete('http://47.113.124.225:8083/User', {params:{stno:editParams.stno}})
								.then(function (response) {
								    console.log(response.data);
							...
							sec_this.$axios.put('http://47.113.124.225:8083/User', formData)
								.then(function (response) {
								    console.log(response.data);
			...
			addSubmit: function (ev) {
				if(this.addForm.stno == '' || this.addForm.password == '' || this.addForm.name == '' || this.addForm.tel== '' 
				|| this.addForm.academy== ''||this.addForm.major == ''|| this.addForm.xuexiao== ''||this.addForm.xueli== '' ||this.addForm.sex== ''){
                    this.$message({
                        message: '存在表单项为空，请填写完整',
                        type: 'warning'
                    });
                    return false
                }
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							sec_this.$axios.post('http://www.xhw8888.com/index/app/saveUserz',qs.stringify(addParams))
								.then(function (response) {
			...
			batchRemove: function () {
				...
				this.$confirm('确认删除选中记录吗？', '提示', {
					...
					for ( var i = 0; i <arr.length; i++){
						...
						_this.$axios.delete('http://47.113.124.225:8083/User', {params:{stno:para.stno}})
					    .then(function (response) {
						})
						.catch(function (error) {
							console.log(error);
